...
async function persistGamesSnapshot(items: FeedGame[], page: number) {
    try {
        await db
            .insert(gamesTable)
            .values(
...
export async function GET(request: Request) {
    ...
    try {
        ...
        if (Array.isArray(data.items) && data.items.length > 0) {
            console.log(`🟡 DB sync queued (page=${page}, items=${data.items.length})`);
            void persistGamesSnapshot(data.items as FeedGame[], page);
        }
